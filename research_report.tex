...
We evaluate ECHO on the NIFTY 50 universe using daily adjusted close prices from 2019 to 2024. For each experiment, we use a target stock \( i \), a prediction horizon \( H \), and neighbor sets proposed by the Orchestrator. The Forecast Service performs a purged 80/20 chronological split with horizon-aware train/test separation to prevent overlap leakage. We train the same \texttt{XGBRegressor} with default hyperparameters across all runs to isolate the effect of neighbor selection. Experiments for each horizon are logged in separate files (\texttt{experiments\_{hH}.json}), analyzed independently into \texttt{analysis\_{hH}.json}, and compared side-by-side in the dashboard. Every run is logged with full provenance. The Analyzer then produces \texttt{analysis.json} per run, which powers the interactive dashboard.
...
We evaluate across multiple horizons (\(H \in \{5, 10, 20\}\) trading days). Each horizon is tracked in a separate experiment run (\texttt{experiments\_\{h5,h10,h20\}.json}), analyzed independently into \texttt{analysis\_\{h5,h10,h20\}.json}, and compared side-by-side via a run-selector dropdown in the dashboard. This allows researchers to assess how neighbor-signal strength degrades or accumulates with horizon length.
...
\begin{table}[htbp]
\centering
\caption{Baseline A vs B vs Ours comparison (same XGBoost regressor and 80/20 chronological split, horizon $H=5$). Lower MAE is better. ``Beats 0'' indicates whether the configuration outperforms the zero-return baseline.}
\label{tab:baseline-abc}
\begin{tabular}{lcccccccc}
\toprule
Target & Sector & A MAE & B MAE & ECHO MAE & Winner & A Beats 0 & B Beats 0 & ECHO Beats 0 \\
\midrule
ITC      & Consumer Defensive & 0.0218 & \textbf{0.0207} & 0.0211 & B & N & N & N \\
HDFCBANK & Financial Services & \textbf{0.0205} & 0.0416 & 0.0207 & A & N & N & N \\
RELIANCE & Energy             & 0.0211 & 0.0360 & \textbf{0.0210} & ECHO & N & N & N \\
TCS      & Technology          & \textbf{0.0241} & 0.0250 & 0.0245 & A & N & N & N \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:baseline-abc} (from the dashboard) visualizes win counts across all evaluated targets. ECHO wins on \texttt{mae\_for\_ranking} in a subset of targets, confirming the value of dynamic, memory-augmented neighbor proposals for specific stocks while other targets perform best with target-only features.
...
The dashboard also shows that while many configurations do not beat the zero-return baseline (a common outcome in noisy daily return data), ECHO finds configurations that match or approach target-only performance while adding interpretable cross-stock signal. The Baseline A vs B vs Ours win counts (A: 23, B: 10, Ours: 23) reveal that target-only features already capture much of the autoregressive structure, but the agent-proposed neighbors consistently outperform static correlation-based neighbors (B), showing that neighbor selection matters and the agent learns to avoid poor neighbor choices.
